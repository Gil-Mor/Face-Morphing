...
def doMorphing(img1, img2, duration, frame_rate, draw_triangles, alpha_blend_method, match_histogram, intermediate_output, video_output):

	[size, img1, img2, points1, points2, list3] = generate_face_correspondences(img1, img2)

	tri = make_delaunay(size[1], size[0], list3, img1, img2)

	generate_morph_sequence(duration, frame_rate, img1, img2, points1, points2, tri, size, draw_triangles, alpha_blend_method, match_histogram, intermediate_output, video_output)
...
if __name__ == "__main__":
	...
	parser.add_argument("-histo", "--match-histogram", action='store_true', help="If True Each frame histogram will be matched to that of the 'target' image (img2). That can make face swapping easier color-wise.")
	...
	doMorphing(image1, image2, args.duration, args.frame, args.draw_triangles, args.alpha_blend_method, args.match_histogram, args.intermediate_output, args.video_output)
